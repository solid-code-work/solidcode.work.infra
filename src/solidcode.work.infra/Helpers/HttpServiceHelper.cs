...
public class HttpServiceHelper
{
    ...
    public async Task<TResponse<T>> GetAsync<T>(string endpoint) where T : class
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return TResponseFactory.Ok(data!, "GET request successful.");
            }
            else
            {
    ...
    public async Task<TResponse<TResponse>> PostAsync<TRequest, TResponse>(string endpoint, TRequest payload)
        ...
        where TRequest : class
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return TResponseFactory.Ok(data!, "POST request successful.");
            }
            else
            {
                var errorMessage = await response.Content.ReadAsStringAsync();
                return new TResponse<TResponse>
                {
                    IsSuccess = false,
                    StatusCode = (int)response.StatusCode,
                    Message = $"HTTP {response.StatusCode}: {errorMessage}",
        ...
        {
            return TResponseFactory.Error<TResponse>(ex.Message);
        }
    }
